"""
XLE 물타기 전략 대시보드
streamlit run dashboard_xle.py --server.port 8506

고빈도 전략: RSI 45/50 → 60/55, GC OFF
- 거래 38회, 물타기 최대 9회, 승률 100%, 수익률 +4.6%
- 에너지 섹터 ETF (석유/가스)
"""
...
def main():
    ...
    st.caption(f"에너지 섹터 ETF (석유/가스) | 마지막 업데이트: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
